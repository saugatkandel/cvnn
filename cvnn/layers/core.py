...
from cvnn.initializers import (
    ComplexGlorotUniform,
    Zeros,
    Ones,
    ComplexInitializer,
    INIT_TECHNIQUES,
...
class ComplexFlatten(Flatten, ComplexLayer):
    def call(self, inputs: t_input):
        ...
        return tf.cast(
            tf.complex(real_flat, imag_flat), inputs.dtype
        )  # Keep input dtype
    ...
    * bias is a bias vector created by the layer
    ...
    def build(self, input_shape):
        if self.my_dtype.is_complex:
            i_kernel_dtype = (
                self.my_dtype
                if isinstance(self.kernel_initializer, ComplexInitializer)
                else self.my_dtype.real_dtype
            )
            i_bias_dtype = (
                self.my_dtype
                if isinstance(self.bias_initializer, ComplexInitializer)
                else self.my_dtype.real_dtype
            )
            ...
            if self.use_bias:
                self.b_r = tf.Variable(
                    ...
                    initial_value=self.bias_initializer(
                        shape=(self.units,), dtype=i_bias_dtype
                    ),
    ...
    def call(self, inputs: t_input):
        ...
        if inputs.dtype != self.my_dtype:
            tf.print(
                f"WARNING: {self.name} - Expected input to be {self.my_dtype}, but received {inputs.dtype}."
            )
            if self.my_dtype.is_complex and inputs.dtype.is_floating:
                tf.print(
                    "\tThis is normally fixed using ComplexInput() "
    ...
    def get_real_equivalent(self, output_multiplier=2):
        ...
        return ComplexDense(
            units=int(round(self.units * output_multiplier)),
            activation=self.activation,
            use_bias=self.use_bias,
            kernel_initializer=self.kernel_initializer,
            bias_initializer=self.bias_initializer,
            kernel_constraint=self.kernel_constraint,
            kernel_regularizer=self.kernel_regularizer,  # MODIFIED CODE ------
            dtype=self.my_dtype.real_dtype,
            name=self.name + "_real_equiv",
        )
    ...
    def get_config(self):
        ...
        config.update({"dtype": self.my_dtype, "init_technique": self.init_technique})
        return config
...
class ComplexDropout(Layer, ComplexLayer):
    ...
    def __init__(
        self, rate: float, noise_shape=None, seed: Optional[int] = None, **kwargs
    ):
        """
    ...
    def get_config(self):
        ...
        config.update(
            {"rate": self.rate, "noise_shape": self.noise_shape, "seed": self.seed}
        )
        return config
...
class ComplexBatchNormalization(Layer, ComplexLayer):
        ...
        if self.my_dtype.is_complex:
            self.gamma_r = tf.Variable(
                ...
                initial_value=self.gamma_initializer(
                    shape=tuple(desired_shape), dtype=self.my_dtype
                ),
                trainable=True,
            )
            ...
            self.moving_mean = tf.Variable(
                ...
                initial_value=tf.complex(
                    real=self.moving_mean_initializer(
                        shape=desired_shape, dtype=self.my_dtype
                    ),
    ...
    def call(self, inputs, training=None):
        if inputs.dtype != self.my_dtype:
            tf.print(
                f"Warning: Expecting input dtype {self.my_dtype} but got {inputs.dtype}. "
        ...
        if training:
            ...
            if self.cov_method == 1:
                ...
                var_20_20 = tfp.stats.covariance(
                    X_20, sample_axis=self.used_axis, event_axis=-1
                )
                valu = int(var_20_20.shape[-1] / 2)
                indices = [
                    ([[i, i], [i, i + valu]], [[i + valu, i], [i + valu, i + valu]])
                ...
                var = var_10_2_2
                """
                centered_inputs = inputs - tf.math.reduce_mean(
                    inputs, axis=self.used_axis, keepdims=True
                )
                Crr = tf.math.reduce_mean(
                    tf.math.real(centered_inputs) ** 2, axis=self.used_axis
                )
                Cii = tf.math.reduce_mean(
                    tf.math.imag(centered_inputs) ** 2, axis=self.used_axis
                )
                Cri = tf.math.reduce_mean(
                    tf.math.real(centered_inputs) * tf.math.imag(centered_inputs),
                    axis=self.used_axis,
                )
                s1 = tf.stack([Crr, Cri], axis=1)
                s2 = tf.stack([Cri, Cii], axis=1)
                var = tf.stack([s1, s2], axis=2)
            ...
            self.moving_var.assign(
                self.moving_var * self.momentum + var * (1.0 - self.momentum)
            )
        ...
        if self.scale:
            if self.my_dtype.is_complex:
                ...
                )  # TODO: Should this be real valued?
            else:
                gamma = self.gamma
            out = gamma * out
        if self.center:
    ...
    def _normalize(self, inputs, var, mean):
        ...
        inv_sqrt_var = tf.linalg.sqrtm(
            tf.linalg.inv(var + self.epsilon_matrix)
        )  # var^(-1/2)  # TODO: Check this exists always?
        # Separate real and imag so I go from shape [...] to [..., 2]
        zero_mean = tf.stack(
            (tf.math.real(complex_zero_mean), tf.math.imag(complex_zero_mean)), axis=-1
        )
        ...
        # Get complex data
        complex_inputs_hat = tf.cast(
            tf.complex(squeeze_inputs_hat[..., 0], squeeze_inputs_hat[..., 1]),
            dtype=self.my_dtype,
        )
        # import pdb; pdb.set_trace()
    ...
    def get_real_equivalent(self):
        return ComplexBatchNormalization(
            axis=self.axis,
            momentum=self.momentum,
            center=self.center,
            scale=self.scale,
            beta_initializer=self.beta_initializer,
            epsilon=self.epsilon_matrix[0],
